...
export const calculateBestRouteService = async ({
  ...
  if (shouldUseDirectPath) {
    try {
     ...
     const quoteAmountInteger = quoteAmount.multipliedBy(10 ** 7).integerValue();

      console.log("calculateBestRouteService: Calling getPairAddress with contracts:", 
        
        { currencyIn: amountAsset.currency, currencyOut: quoteAsset }, '==', quoteAmountInteger.toString()
      );
      ...
      const toReturn: BuildTradeReturn =
        tradeType === TradeType.EXACT_INPUT
          ...
          : {
            ...
            trade: {
              amountOut: BigInt(new BigNumber(amount).multipliedBy(10 ** 7).integerValue().toString()),
...
export const getDerivedSwapInfoService = async (
  ...
  console.log("🚀 ~ state:CASE SWAP CURRENCY", state, )

  // Fetch input and output tokens using findTokenService
  const inputCurrency = await findTokenService(inputCurrencyId ?? undefined, tokensAsMap, sorobanContext, 'getDerivedSwapInfoService');
  
  const outputCurrency = await findTokenService(outputCurrencyId ?? undefined, tokensAsMap, sorobanContext, 'getDerivedSwapInfoService');
  ...
  if (account && tokensArray && horizonAccount) { // Use passed horizonAccount
    const balancesResult = await tokenBalances(account, tokensArray, sorobanContext, horizonAccount, undefined, 'getDerivedSwapInfoService');
  ...
  if (parsedAmount && inputCurrency && outputCurrency) {
    ...
    console.log("getDerivedSwapInfoService: Calling calculateBestRouteService with input and output currencies:", inputCurrency, outputCurrency, parsedAmount, typedValue);
    try {
      const route = await calculateBestRouteService({
        amountAsset: parsedAmount,
        quoteAsset: outputCurrency,
        amount: typedValue, // Use typedValue for the amount
      ...
      trade = {
        trade: {
          ...route, // Include other properties from the route
