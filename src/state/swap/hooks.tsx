...
export function useDerivedSwapInfo(state: SwapState) {
  ...
  const { token: inputCurrency } = useToken(inputCurrencyId!, 'useDerivedSwapInfo');

  //
  const { token: outputCurrency } = useToken(outputCurrencyId!, 'useDerivedSwapInfo');
  ...
  useEffect(() => {
    if (account) {
      ...
      tokenBalances(account, tokensArray, sorobanContext, horizonAccount, undefined, 'useDerivedSwapInfo').then((res) => {
        if (res) {
